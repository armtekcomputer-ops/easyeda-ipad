...
Updated: 2026-09-21T15:27:00Z. All timestamps use UTC ISO 8601.
...
| chat-20260921T152500Z-agent3-rel005 | Agent 3 — transport reliability | REL-005 | in_progress | 2026-09-21T15:27:00Z | `work/REL-005/chat-20260921T152500Z-agent3-rel005` | Implement R5 handshake/pong/reconnect and relay status restoration plus R6 bounded generation-safe pending relay tracking; add focused tests and CI |
...
| REL-005 | P2 | R5 connection deadlines/recovery/status + R6 bounded pending requests | in_progress | chat-20260921T152500Z-agent3-rel005 | Scope includes Worker relay-status persistence because browser reconnect otherwise cannot restore `edaConnected/localBridgePort` immediately. Implement focused reliability tests only; broader protocol suite remains TEST-008. |
...
| REL-005 | chat-20260921T152500Z-agent3-rel005 | `src/lib/gateway.ts`; `src/lib/gateway.test.ts`; `companion/cloud-agent.mjs`; `companion/relay-pending.mjs`; `companion/relay-pending.test.mjs`; `worker/index.ts` | active | PR reaches review/merge or explicit handoff; then synchronize board and release |
...
- scope: R5 browser gateway connection deadline/pong watchdog/bounded reconnect plus Worker-backed companion-status restoration; R6 PC companion pending relay TTL/max-in-flight/generation-safe cleanup
- status restoration design: persist `edaConnected/localBridgePort` in the live companion WebSocket attachment so newly reconnected iPad sockets immediately receive current relay state
- focused tests allowed only for REL-005 regressions; broader transport suite stays TEST-008
- live tested: no
- next: sync branch with this coordination commit, implement reserved paths, run CI, open PR, then move task to review if green
